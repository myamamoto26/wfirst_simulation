...
def plot_3points(num, dirr1, dirr2, dirr3, dirr4):
    ...
    chiquad2=0
    for j in range(3):
        chilin1 += ((e002[j] - linefit1[j])**2)/np.cov(e002)
        chiquad1 += ((e002[j] - quadfit1[j])**2)/np.cov(e002)
        chilin2 += ((e005[j] - linefit2[j])**2)/np.cov(e005)
        chiquad2 += ((e005[j] - quadfit2[j])**2)/np.cov(e005)
