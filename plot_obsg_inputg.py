...
def main(argv):
    ...
    ax1.plot([-0.02, 0.02], mean_randdiff, 'o', c='b', label='g1, perfectly randomized orientation angles')
    ax1.errorbar([-0.02, 0.02], mean_randdiff, yerr=error_randg1, c='b', fmt='o')
    ...
    ax1.plot([-0.02, 0.02], mean_rand2diff, 'o', c='g', label='g1, slightly randomized orientation angles')
    ax1.errorbar([-0.02, 0.02], mean_rand2diff, yerr=error_rand2g1, c='g', fmt='o')

    ax1.set_xlabel('input shear', fontsize=16)
    ax1.set_ylabel("\u0394\u03B3", fontsize=16)
    ax1.set_title('Mean difference in measured shapes (random orientation angles, offsets=45 degrees)', fontsize=11)
